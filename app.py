...
#                     AND (Hough circle off-centre >0.25
#                          OR contrast<-5 with lap_in>1000 = palm lines)
    ...
    for p2 in [40, 30, 22, 18, 14]:
        ...
        if cc is not None:
            ...
            if interior:
                ...
                return (out_m - in_m, abs(dom[0] / w - 0.5), float(np.std(ins)), interior, lap_in)
    return None
...
def is_eye_image(image_path):
    try:
        ...
        # but contain no iris ring.  Two independent sub-checks:
        #   (a) Off-centre Hough circle (cx_offset > 0.25)
        #       → the dominant circle is a background blob, not an iris
        #   (b) Negative contrast (inside brighter than outside) AND high internal
        #       Laplacian (>1000) → palm lines / texture, not an iris/pupil hollow
        # Using std alone was too aggressive — many white-cataract eyes have low
        # std inside their Hough circle, just like palms.
        if skin_frac > HAND_SKIN_THRESH and warm_frac > HAND_WARM_THRESH and n_casc == 0:
            if hough_info is not None:
                contrast, cx_off, _, _, lap_in = hough_info
                no_real_iris = (
                    (cx_off > IRIS_CX_THRESH)                      # off-centre blob
                    or (contrast < -5 and lap_in > 1000)           # palm-line texture
                )
            else:
                no_real_iris = True   # flesh-dominated + no circle = body part
            if no_real_iris:
                return (
                    False,
        ...
        if hough_info is not None and (h, w) == (hs, ws):
            interior = hough_info[3]   # index 3 = circles list; reuse — zero extra computation
        else:
